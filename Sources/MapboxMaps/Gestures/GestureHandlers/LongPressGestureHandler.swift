...
final class LongPressGestureHandler: GestureHandler, UIGestureRecognizerDelegate {
    ...
    @objc private func handleGesture(_ gestureRecognizer: UITapGestureRecognizer) {
        ...
        case .began:
            (map as? MapboxMap)?.dispatch(event: CorePlatformEventInfo(type: .longClick, screenCoordinate: point))
            (map as? MapboxMap)?.dispatch(event: CorePlatformEventInfo(type: .dragBegin, screenCoordinate: point))

        case .changed:
            (map as? MapboxMap)?.dispatch(event: CorePlatformEventInfo(type: .drag, screenCoordinate: point))

        case .ended, .cancelled:
            (map as? MapboxMap)?.dispatch(event: CorePlatformEventInfo(type: .dragEnd, screenCoordinate: point))
